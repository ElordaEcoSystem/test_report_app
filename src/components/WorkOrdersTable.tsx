...
export default function WorkOrdersTable() {
  ...
  return (
    <div className="flex flex-col h-[100dvh] overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-24 md:pb-4 w-full">
        {/* Desktop: Таблица */}
        <div className="hidden md:block">
          <Card>
            <CardContent className="overflow-x-auto p-4">
          ...
          <div className="md:hidden flex flex-col gap-2 mt-2">
            {orders.map((order) => (
              <Card key={order.id} className="p-4 max-w-full overflow-hidden">
                ...
                <div className="font-medium text-lg break-words">{order.object}</div>
                <div className="text-sm mt-1 break-words">
                  <strong>Автор:</strong> {order.createdBy?.name ?? '—'}
                </div>
                <div className="text-sm break-words">
                  <strong>Выполненные работы:</strong> {order.completedWorkText ?? '—'}
                </div>
              </Card>
            ))}
          </div>
                        ...
                        <TableRow key={order.id}>
                          ...
                          <TableCell>
                            {order.photoUrl ? (
                              ...
                              <div className="w-16 h-16 bg-gray-200 rounded-md flex items-center justify-center text-xs text-muted-foreground border">
                                —
                              </div>
                            )}
                          </TableCell>
